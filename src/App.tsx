...
function AppRoutes() {
  ...
  // Landing page: full-screen, no nav chrome
  if (location.pathname === "/") return <LandingPage />;

  if (loading) return <Loading progress={progress} error={error} />;

  const isExamOrFocus = location.pathname === "/exam" || location.pathname === "/focus";

  return (
    <div className="min-h-screen" style={{ background: "#0B0F14" }}>
      {/* Floating pill nav — hidden on exam/focus */}
      <FloatingNav onMenuOpen={() => setSideOpen(true)} />

      {/* Legacy bottom tabs — exam/focus only (minimal) */}
      {!isExamOrFocus && (
        // Mobile bottom tabs — exam page manages its own HUD
        <NavBar qCount={questions.length} />
      )}

      {/* Global side panel — hamburger overlay */}
      <SidePanel
        open={sideOpen}
